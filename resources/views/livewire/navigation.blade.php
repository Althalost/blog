<nav class="bg-gray-800" x-data="{ open: false }">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              ...
              @foreach ($categories as $category)
                <a href="{{route('posts.category', $category)}}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">
                  {{$category->name}}
                </a> 
              @endforeach
    ...
    <div class="sm:hidden" x-show="open" x-on:click.away=" open = false" id="mobile-menu">
      <div class="space-y-1 px-2 pb-3 pt-2">
        ...
        @foreach ($categories as $category)
          <a href="{{route('posts.category', $category)}}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">
            {{$category->name}}
          </a>  
        @endforeach
